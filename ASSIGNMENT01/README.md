...
# Second Website: NOAA Fisheries: Protecting Marine Life
...
Good: Since there is no login or user registration, the website doesn't keep user information. In the homepage, there are interactive boxes that get bigger size when hovered over. Also, under the navigation bar the mouse turns into a pointer position and the text is underlined. There is actually a spinning wheel when going to the feedback section. 

Bad: There aren't much indicators such as progress bars or spinning circles that informs the user of an action being made. 
...
Bad: The consistency of the nav bar isn't best. You have a couple options named "Find a Species" and "Fishing & Seafood" while others are just called "Regions" and "Environment" which isn't that indicative. Also, the news subheader is under the About Us page which is peculiar.

Recommendations: Re-look at the naming of headers as well as the organization of the subheaders.
...
Good: User input here is only really in two sections: the feedback page and the newsletter signup. Both are pretty well setup. 
...
Bad: There is two nav bars actually. One in gray at the very top of the website and a second bigger one a little more to the center. Standards wise this is very bad since you have two nav bars with different schemes, colors and fonts. 

Recommendations: Get rid of top nav bar and be more consistent throughout the site.
...
Good: The sub headers are mostly well distributed in order to make sure the user knows where to go. They also have a big search bar which makes it pretty nice for the user to search up stuff. Most of the headers are actually very organized and you can access every part of the site from it.
...
Bad: Expert users may want to search something up in the search bar since they already know what they want. However, the search bar doesn't give the most optimal results. I tested the program by typing tabs on the site and it would always give back in the 5th or 6th result.

Recommendations: Would be a great improvement if the search bar returned more adequate results.
...
Bad: The website is a mess with this in particular. The homepage has so much going on and feels like information is very cluttered. After the news section there is alot of "Rules", "Bulletins", "Events" and "Opportunities" which are scattered all over the place. The footer is also a mess with alot of different indirectly repititve links as well as the header which has another nav bar with different styles.

Recommendations: Unclutter the website by concealing info and distributing it appropriately. Reorganzie the nav bar(s) and the footers.
...
Good: In the contact form, if I type in a wrong email or wrong number the site tells me automatically that I'm doing an error and gives a pretty good error message to user.
...
Good: There is three elements here: "The Search Bar", "The Site Index" and a "Tutorial on how to Navigate the Site". The Search Bar is the least helpful but is a resource but the Site Index is extremely helpful because it organizes most links in a pretty good directory fashion. The tutorial is youtube video that helps novice users out and is helpful.

Bad: Not many bad things however the site index and tutorial were hard to find, the tutorial is also under news. 

Recommendations: Make the tutorial and site index easier to find for people to navigate the website.
